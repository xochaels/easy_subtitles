...
from sub_download import (getUserToken, destroyUserToken, searchSubtitles,
                          getSubtitlesInfo, downloadSubtitles, getUserInfo)
from changetime import shift_subtitle
from easy_translate import translate_subtitles
...
def sync_sub(downloaded_en_dir, change_time_dir, from_eps, to_eps, time_change, input_path_time):
    for eps in range(from_eps, to_eps + 1):
        change_time_path = os.path.join(change_time_dir, f"episode_{eps}")
        os.makedirs(change_time_path, exist_ok=True)
        ...
        if input_path_time == 1:
            shift_subtitle(os.path.join(download_sub_path, f'episode_{eps}.srt'),
                           os.path.join(change_time_path, f'episode_{eps}.srt'), milliseconds=time_change)
        else:
...
def translate(downloaded_en_dir, change_time_dir, translated_dir, from_eps, to_eps, target_language, input_path_trans):
    for eps in range(from_eps, to_eps + 1):
        ...
        if input_path_trans == 1:
            ...
            translate_subtitles(os.path.join(change_time_path, f"episode_{eps}.srt"),
                                os.path.join(translate_dir_path, f"episode_{eps}.srt"), target_language=target_language)
            time.sleep(3)
...
if __name__ == "__main__":
    ...
    translated_dir = os.path.join(os.getcwd(), 'srt_file', 'translated_srt')
    downloaded_en_dir = os.path.join(os.getcwd(), 'srt_file', 'srt_downloaded_en')
    change_time_dir = os.path.join(os.getcwd(), 'srt_file', 'change_time')
    ...
    if mode == 1:
        ...
        make_writable_and_remove(change_time_dir)
        sync_sub(downloaded_en_dir, change_time_dir, from_eps, to_eps, time_change, input_path_time)

    else:
        make_writable_and_remove(translated_dir)
        translate(downloaded_en_dir, change_time_dir, translated_dir, from_eps, to_eps, target_language,
                  input_path_trans)
